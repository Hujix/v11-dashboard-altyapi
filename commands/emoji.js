		...
		message.channel.send(embed).then(async function (sentEmbed) {
			...
			reactions.on("collect", async function (reaction) {
				...
				embed = new Discord.RichEmbed()
					.setTitle("Emojis Added to the Server")
					.setDescription(`:${emojis[page - 1].name}:`)
					.setImage(emojis[page - 1].url)
					.setFooter(`Sayfa ${page}/${totalpages} | Emoji ID: ${emojis[page - 1].id}`)
					.setColor("BLUE");
			...
			reactions.on("end", () => sentEmbed.edit("Interactive command expired: 5 minutes passed."));
		}).catch(() => {
			message.reply("An error occurred while trying to send this embed.").catch(() => {
...
module.exports.help = {
  name: 'emoji',
  category: "General",
  description: 'Shows Emojis on Server',
  usage: 'emoji'
};
//XiR